...
# from model_multiclass_large import *
from LeNet5_new import *
...
#         z = x[:,i].unsqueeze(-1).repeat(1, N)
#         z = torch.transpose(z,0,1)        
#         temp=torch.sum(mvnpdf(z.cpu().numpy(),mu.cpu().numpy(),Sig.numpy()))/N
#         fp_mixt.append(temp)
#     #return fp_mixt.clone().detach()
...
#     xRinv = torch.zeros(n,d)
#     logSqrtDetSigma = torch.zeros(n)
#     for i in range(n):
#         R = torch.cholesky(Sigma[:,:,i], upper = True)
#         X0 = X-Mu
#         xRinv[i,:] = X0[i,:].cpu() @ torch.inverse(R).cpu()
#         logSqrtDetSigma[i] = torch.sum(torch.log(torch.diag(R)))
#     # The quadratic form is the inner products of the standardized data
#     quadform = torch.sum(xRinv**2, 1)

#     y = torch.exp(-0.5*quadform - logSqrtDetSigma - d*np.log(2*np.pi)/2)
...
def BNN_posterior(N, K, N_resampled,population, weights, x_test, y_test1, tp, is_binary):
    #is_binary = 1
    #N_resampled = 1000#number of particles
    m = nn.Sigmoid()
    L =tp['L']
...
def evaluate_target_general_new(vector,tp):
    [Weight_0,B_0] = Vec2param(vector, tp)
    [W_g,B_g,l] = calculate_grad(Weight_0,B_0,tp)
    return W_g,B_g,l


def evaluate_target_general_new_diag(vector,tp,x_train,y_train):
    [Weight_0,B_0] = Vec2param(vector, tp)
